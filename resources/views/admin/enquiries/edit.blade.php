    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.enquiries.update", [$enquiry->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mobile') ? 'is-invalid' : '' }}" type="text" name="mobile" id="mobile" value="{{ old('mobile', $enquiry->mobile) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('enquiry_type') ? 'is-invalid' : '' }}" name="enquiry_type" id="enquiry_type" required>
                    ...
                    @foreach(App\Models\Enquiry::ENQUIRY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('enquiry_type', $enquiry->enquiry_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('whatsapp_update') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="whatsapp_update" id="whatsapp_update" value="1" {{ $enquiry->whatsapp_update || old('whatsapp_update', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('privacy') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="privacy" id="privacy" value="1" {{ $enquiry->privacy || old('privacy', 0) === 1 ? 'checked' : '' }}>
